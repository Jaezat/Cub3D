...
void paint_pixel(t_env *env, int x, int y)
{
	...
	while (px_y < MINIMAP_TILE_SIZE)
	{
		...
		while (px_x < MINIMAP_TILE_SIZE)
		{
			screen_x = (x - (int)env->minimap.cam.x) * MINIMAP_TILE_SIZE + px_x + offset;
			screen_y = (y - (int)env->minimap.cam.y) * MINIMAP_TILE_SIZE + px_y + offset;
			my_mlx_pixel_put(&env->umlx.img_data, screen_x, screen_y, env->minimap.cam.c);
			px_x++;
		}
		px_y++;
	}
